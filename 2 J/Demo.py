...
class Demo:
	...
	def ajax(self, chartType):
		...
		element.values = [math.sin(float(x)/5) for x in range(0, 30)]
		element.on_show.type = "pop-up"
		element.on_show.cascade = 1
		element.on_show.delay = 0.5
		element.fill = "#333333"
		element.fill_alpha = 0.7

		# Create chart object
		chart = Chart()
		chart.title.text = "Different charts"
